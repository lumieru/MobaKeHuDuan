...
public static class AutoBuilder {
	...
	static void PerformAndroidBuild() {
		...
		string error = BuildPipeline.BuildPlayer (CollectScenePaths(), targetDir, BuildTarget.Android, opt );
		if (error != null && error.Length > 0) {
			throw new IOException("Build Failed: "+error);
		}
	}
	...
	static void PerformUbuntuBuild() {
        ...
        var targetDir = "../../zc/Scripts/windows";
		string error = BuildPipeline.BuildPlayer (CollectScenePaths(), targetDir, BuildTarget.StandaloneLinux64, opt );
		if (error != null && error.Length > 0) {
			throw new IOException("Build Failed: "+error);
		}
	}
